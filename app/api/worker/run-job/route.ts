...
export const POST = verifySignatureAppRouter(async (req: Request) => {
  ...
  try {
    ...
    const inserts = urls.map((url, idx) => ({ job_id: jobId, image_url: url, index: idx }));

    const { error: outErr } = await supabaseAdmin.from("job_outputs").insert(inserts);
    ...
    // Refund based on how we charged this job.
    ...
    const charge = (ledgerRows ?? []).find((r: any) => r.delta === -COST_PER_JOB);
    ...
    if (userRow) {
      ...
      if (charge?.reason === "free_quota_used") {
        await supabaseAdmin
          .from("users")
          .update({ free_used: Math.max(0, userRow.free_used - COST_PER_JOB) })
          .eq("id", job.user_id);
      }

      await supabaseAdmin
        .from("credit_ledger")
        .insert({ user_id: job.user_id, job_id: jobId, delta: +COST_PER_JOB, reason: "job_refund_failed" });
    }
